...
import { FiUser, FiEdit, FiTrash2, FiMessageCircle } from "react-icons/fi";
...
export default function CommunityBoardPostDetail(): JSX.Element {
  ...
  const [reportTypeCodeId, setReportTypeCodeId] = useState<number>(1); // 1=게시글, 2=댓글
...
=======
    ...
    if (!user?.id) {
>>>>>>> ca0f8a94cf5de47149ff9e15fa00aa0c0eb7ba75
      toast({
        title: "인증 오류",
...
>>>>>>> ca0f8a94cf5de47149ff9e15fa00aa0c0eb7ba75
      ...
      if (error.response?.status === 403) errorMessage = "게시글을 삭제할 권한이 없습니다.";
        ...
        <div className="w-full max-w-[1000px] px-4 py-10">
          ...
          <div className="bg-white shadow rounded-xl ring-1 ring-gray-100 px-8 py-10 space-y-8">
            ...
            <div className="mb-6 border-b pb-4 flex items-center justify-between">
              ...
              {/* 게시글 신고 버튼 */}
              <button
                  className="flex items-center px-3 py-1 border rounded-md text-sm gap-1 hover:bg-red-50 text-red-500"
                  onClick={() => {
                    setReportTargetId(post.postId);
                    setReportTypeCodeId(1); // 게시글 신고: 1
                    setShowReport(true);
                  }}
              >
                신고
                    ...
                    comments.map((comment) => (
                        <div key={comment.commentId} className="border rounded-lg p-4 w-full max-w-[1000px]">
                          <div className="flex items-center text-sm text-gray-600 mb-1 gap-2">
                            ...
                            {/* 댓글 신고 버튼 */}
                            <button
                                className="ml-auto flex items-center px-2 py-1 border rounded-md text-xs gap-1 hover:bg-red-50 text-red-500"
                                onClick={() => {
                                  setReportTargetId(comment.commentId);
                                  setReportTypeCodeId(2); // 댓글 신고: 2
                                  setShowReport(true);
                                }}
                            >
                              신고
