...
// import dayjs from "dayjs"; // createdAt 포맷 변환용 필요시
                        ...
                        return (
                            ...
                            >
                              <div className={`flex items-end gap-2 max-w-[70%] ${isMine ? "flex-row-reverse" : ""}`}>
                                ...
                                <div>
                                  <div className={`flex items-center gap-2 ${isMine ? "justify-end flex-row-reverse" : ""}`}>
                                    <span className="font-medium text-sm">{msg.senderNickname}</span>
                                    <span className="text-xs text-gray-400">
                                {/* dayjs 등으로 포맷 원하면 여기에 적용 */}
                                      {typeof msg.createdAt === "string"
                                          ? msg.createdAt.replace("T", " ").slice(0, 16)
                                          : msg.createdAt}
                              </span>
                                  </div>
                                  <div className={`mt-1 px-4 py-2 rounded-2xl text-base break-words ${isMine ? "bg-blue-100 text-blue-900 text-right" : "bg-gray-100 text-black text-left"}`}>
                                    {msg.content}
                                  </div>
                                </div>
                              </div>
